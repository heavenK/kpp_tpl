...
function get_pay_url($charge_type, $pay_amount, $payment_config, $subject, $order_id, $model_id = null, $obj_id = null, $service = null, $sign_type = 'MD5', $method = 'form') {
	...
	$parameter = array ("service" => $service, "partner" => $payment_config ['seller_id'], "return_url" => $_K ['siteurl'] . '/payment/alipayjs/return.php', "notify_url" => $_K ['siteurl'] . '/payment/alipayjs/notify.php', "_input_charset" => CHARSET, "subject" => $subject, "body" => $body, "out_trade_no" => "charge-{$charge_type}-{$uid}-{$obj_id}-{$order_id}-{$model_id}-".time(), "total_fee" => $pay_amount, "payment_type" => "1", "show_url" => $_K ['siteurl'] . "/index.php?do=user&view=finance", "seller_email" => $payment_config ['account'],"extend_param"=>"isv^kk11");
