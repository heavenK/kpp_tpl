...
$third_nav = array ("basic" => array ($_lang['basics'], $_lang['realname_ect_setting'] ), "contact" => array ($_lang['contact_type'], $_lang['qq_msn_ect_setting'] ), "skill" => array ('技能标签', $_lang['skills_setting'] ) );
if($user_type){
	switch ($user_type){
		case 1:
...
switch ($opp) {
	case "basic" :
		...
		if($user_info['indus_pid']){
			$indus_arr = kekezu::get_industry($user_info['indus_pid']);
		}
