...
switch ($auth_step) {
	case "step1" :
		...
		$bind_bank = kekezu::get_table_data("*","witkey_member_bank"," uid = '$uid' and bind_status='1'",'','','','bank_id');
		  ...
		  }elseif($ac=='del_auth'&&$bank_id){
		  	$res = db_factory::execute(sprintf("delete a.*,b.* from %switkey_auth_bank a  left join %switkey_member_bank b on a.bank_id = b.bank_id where a.uid=%d and a.bank_id = %d ",TABLEPRE,TABLEPRE,$uid,$bank_id));
		  	$res and kekezu::show_msg('消息提示','index.php?do=user&view=payitem&op=auth&auth_code=bank&step=step4&show=list&#userCenter',1,'删除成功','alert_right') or kekezu::show_msg('消息提示',$ac_url."#userCenter",1,'删除失败','alert_error');
		  }
		break;
	case "step2" :
		if(!$bank_id){
	...
	case "step3" :
        ...
        if(isset($check_code)&&$check_code){
		 	...
		 	if (floatval ( $check_code ) ==$pay_cash&&$pay_cash!=0.00) {
		 		$res = 1;
		 	}else{
		 		$res = '输入金额不正确哦';
		 	}
