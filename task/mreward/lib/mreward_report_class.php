		...
		if($op_result ['action']){
			switch ($op_result ['task']) {
				...
				case 2:
					...
					if (!$bid_count) {
						$this->_task_obj->auto_choose ( $prize_date ['count'] ['prize_1'], $prize_date ['count'] ['prize_2'], $total_prize_count ); 
					} else {
						$this->_task_obj->set_task_status ( 5 ); 
						$this->_task_obj->set_task_sp_end_time ();
					}
					$this->process_notify ( 'pass', $this->_report_info, $this->_user_info, $this->_to_userinfo, $op_result ['result'] ); 
					$res = $this->change_status ( $this->_report_id, '4', $op_result, $op_result ['result'] ); 
					break;
