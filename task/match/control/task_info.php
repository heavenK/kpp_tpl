...
switch ($op) {
	...
	case "work_hand" : //威客抢标
		...
		if ($sbt_edit) {
			$task_obj->work_bid ( $con,$quote,$tar_content,$cycle );
		}
		...
		die ();
		break;
	case "work_edit"://稿件编辑修改报价
		$title = '修改报价';
		if($sbt_edit){
			$task_obj->work_edit ($work_id,$quote,$tar_content,$cycle);
		}else{
			//$work_info = $task_obj->get_work($work_id);
			$work_detail_info = $task_obj->get_match_work($work_id);
			require keke_tpl_class::template ("task/match/tpl/" . $_K ['template'] . '/match_work');
			die();
		}
		break;
	case "work_give_up"://放弃投标
		$task_obj->work_give_up();
		break;
	case "work_cancel"://淘汰投标
		$task_obj->work_cancel();
		break;
	case "task_host" : //托管
		$title = $_lang['match_task_host'];
		if($sbt_edit){
			$task_obj->task_host ($host_cash);
		}
		$limit = $task_obj->_host_amount;
		require keke_tpl_class::template ("task/match/tpl/" . $_K ['template'] . '/match_work');
		die ();
		break;
	...
	case "work_over"://确认完工
		...
		if($sbt_edit){
			$task_obj->work_over($tar_content, $file_id,$file_name,intval($modify));
		}
		require keke_tpl_class::template ("task/match/tpl/" . $_K ['template'] . '/match_work');
		die ();
		break;
	case "task_accept"://工作验收
		$task_obj->task_accept();
		break;
	case "send_notice"://发送提醒
		$task_obj->send_notice($type);
		break;
