...
class preward_task_class extends keke_task_class {
	...
	public function get_work_info($w = array(), $order = null, $p = array()) {
		...
		$where .= "   order by (CASE WHEN  a.work_status!=0 THEN work_id ELSE 0 END) desc,a.work_id desc ";
		if (! empty ( $p )) {
		...
		if ($this->check_if_can_hand ( $url, $output )) {
			if ($this->check_work_if_standard ( 'hand' )) {
				...
				if ($work_id) {
					...
					kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang ['hand_work_fail_and_operate_agian'],"alert_error");
				}
			} else {
				kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang ['hand_work_fail_for_the_work_full'],"alert_error");
			}
		}
	}
				...
				case 0 :
					if ($to_status == 6) {
						if ($this->check_work_if_standard ( 'hege' )) {
							...
							kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id."&view=work",1,$_lang ['task_hg_work_full_and_not_operate_bid_work'],"alert_error");
						}
					} else {
						return true;
					}
					break;
				case 6 :
					kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id."&view=work",1, $_lang ['task_bid_work_full_and_not_operate_choose_work'],"alert_error");
					break;
				case 7 :
					kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id."&view=work",1, $_lang ['task_not_recept_work_full_and_not_operate_choose_work'],"alert_error");
					break;
				case 8 :
					kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id."&view=work",1,  $_lang ['task_not_operate_work_and_not_operate_choose_work'],"alert_error");
					break;
			}
		} else {
	...
	public static function get_work_status() {
		global $_lang;
		return array ('0' => $_lang ['wait_choose'],'6' => $_lang ['hg'], '7' => $_lang ['not_recept'], '8' => $_lang ['task_can_not_choose_bid'] );
	}
