...
$payitem_arr = keke_payitem_class::get_payitem_info('employer','dtender'); 
...
$task_config = $release_obj->_task_config; 
$min_day =$task_config['bid_min_time'];
$min = date("Y-m-d",time()+ 24*3600*$min_day);
$cash_cove = $release_obj->_cash_cove;
$max =$task_config['bid_max_time']  ;
...
switch ($r_step) { 
	...
	switch ($ajax) {
			case "save_payitem" : 
				$release_obj->save_pay_item ($item_id, $item_code, $item_name, $item_cash, $std_cache_name ,$item_num,$item_type);
				break;
			case "rm_payitem" :	
		...
		if (kekezu::submitcheck($formhash)) {
			...
			$total_cash = $release_obj->get_total_cash ( $task_config['deposit'] ); 
