...
function plan_complete(plan_id,plan_step){
	if(check_user_login()){
		var url = basic_url +"&op=plan_complete&plan_id="+plan_id+'&plan_step='+plan_step;
...
function plan_confirm(plan_id,plan_step){
	if(check_user_login()){
		var url = basic_url +"&op=plan_confirm&plan_id="+plan_id+'&plan_step='+plan_step;
		formSub(url,'url',false);return false;
//		$.post(url,{plan_id:plan_id,plan_step:plan_step},function(json){
//			if(json.status==1){
...
function add_task_plan(){	
	var i = parseInt($("#plan_add b:last").html());	
	var k = i+1;	
	if(k>5){
		showDialog(L.t_work_plan_stage_limit,'error',L.operate_notice);return false;
	}else{		
		var append_html = "<div id=\"plan_step_"+k+"\" name=\"plan_step_"+k+"\">"+
		"<div class=\"rowElem clearfix\">"+
		"<div class=\"col6\">" +
		"<p><label>"+L.t_plan_amount+"</label><span class=\"ml_5\">(元)</span></p>"+
		"<input type=\"text\" size=\"11\" name=\"plan_amount[]\" class=\"txt_input\" id=\"plan_amount_"+k+"\" value=\"\" onkeyup=\"clearstr(this)\" limit=\"required:true;type:float\" maxlength=\"5\" msg=\""+L.t_plan_amount_fill_error+"\" tilte=\""+L.t_fill_in_plan_amount+"\" msgArea=\"span_plan_cash_"+k+"\">"+
		"</div>"+
		"<div class=\"col6\">" +
		"<p><label>"+L.t_start_time+"</label></p>"+
		"<input type=\"text\" size=\"11\" name=\"start_time[]\" class=\"txt_input\" id=\"start_time_"+k+"\" onkeyup=\"clearstr(this)\" limit=\"required:true;type:date;than:end_time_"+i+"\" maxlength=\"12\"  onclick=\"showcalendar(event, this, 0)\" msg=\""+L.t_start_time_fill_error+"\" tilte=\""+L.t_fill_in_start_time+"\" msgArea=\"span_plan_cash_"+k+"\">"+
		"</div>"+
		"<div class=\"col6\">" +
		"<p><label>"+L.t_end_time+"</label></p>"+
		"<input type=\"text\" size=\"11\" name=\"end_time[]\" class=\"txt_input\" id=\"end_time_"+k+"\" onkeyup=\"clearstr(this)\" limit=\"required:true;type:date;than:start_time_"+k+"\" maxlength=\"12\"  onclick=\"showcalendar(event, this, 0)\" msg=\""+L.t_end_time_fill_error+"\" tilte=\""+L.t_fill_in_end_time+"\" msgArea=\"span_plan_cash_"+k+"\">"+
		"</div>"+
		"<div class=\"col2\">" +
		"<p><label>"+L.t_work_target+"</label></p>"+
		...
		"<button type=\"button\"  value=\""+L.del+"\" id=\"del_plan\" name=\"del_plan\" onclick=\"del_task_plan("+k+");\" >"+L.del+"</button>"+
		"<span id=\"span_plan_cash_"+k+"\"></span>" +
	"</div><b style=\"display:none;\">"+k+"</b>" +
		"</div>"+
