...
abstract class keke_task_class {
	...
	public function set_task_delay($delay_day, $delay_cash, $trust_response = false) {
		...
		if ($this->check_if_over_delay ( $delay_day, $delay_cash )) {
			$task_info = $this->_task_info;
				...
				if ($delay_cash > $mycredit + $mycash) {
					...
					if ($delay_id) {
						...
						kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,"操作成功","alert_right");
					} else {
						kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,"操作失败","alert_error");
					}
				}
		}
	}
	...
	public function set_task_reqedit($ext_desc) {
		...
		if ($this->check_if_can_reqedit ( $url, $output )) {
			...
			if ($res) {
				kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang['task_need_add_edit_success'],"alert_right");
			} else {
				kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang['task_need_add_edit_fail'],"alert_error");
			}
		}
	}
	...
	public function set_report($obj, $obj_id, $to_uid, $to_username, $report_type, $file_name, $desc,$reason) {
		global $_lang;
		$this->_uid != $this->_guid and $user_type = '1' or $user_type = '2';
		$res = keke_report_class::add_report ( $obj, $obj_id, $to_uid, $to_username, $desc, $report_type, $this->_task_status, $this->_task_id, $user_type, $file_name ,$reason);
	}
			...
			if ($this->_process_can ['delay']) { 
				...
				if ($delay_count < $rule_count) { 
					if ($delay_cash < $real_min) { 
						kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang['the_delay_money_less_min'] . $real_min . $_lang['yuan_and_delay_fail'],"alert_error");
						return false;
					} elseif ($delay_day > $max_day) {
						kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1, $_lang['the_delay_days_more_max'] . $max_day . $_lang['tian_and_delay_fail'],"alert_error");
						return false;
					} else {
						return true;
					}
				} else {
					kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1, $_lang['task_delay_times_has_reached'],"alert_error");
					return false;
				}
			} else {
				kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1, $_lang['task_no_ongoing_no_delay'],"alert_error");
				return false;
			}
		}
	}
	...
	public function check_if_can_hand() {
		global $_K;
		global $_lang;
		$this->_priv ['work_hand'] ['pass'] or kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang['no_submit_work_rights'].$this->_priv ['work_hand'] ['notice'],"alert_error");
		if ($this->_process_can ['work_hand']) {
				return true;
		} else {
			kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id,1,$_lang['current_status_can_not_submit'],"alert_error");
		}
	}
	public function check_if_can_choose($url = '', $output = 'normal') {
		...
		if ($this->_process_can ['work_choose']) {
			...
			kekezu::show_msg($_lang['operate_tips'],"index.php?do=task&task_id=".$this->_task_id."&view=work",1,$_lang['current_status_can_not_choose'],"alert_error");
		}
	}
