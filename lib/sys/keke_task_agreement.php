...
abstract class keke_task_agreement {
	...
	public function agreement_stage_one($user_type, $url = '', $output = 'normal') {
		...
		kekezu::keke_show_msg ( $url, $notice, $type, $output );
	}
	...
	public function upfile_confirm($file_ids, $url = '', $output = 'normal') {
		...
		if($res){
			kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['source_file_success'],"alert_right");
			
		}else{
			kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['source_file_fail'],"alert_error");
			
		}
		
	}
	
	public function accept_confirm($url,$trust_response = false) {
		global $uid;
		global $_lang;
		$agree_info = $this->_agree_info;
		$uid != $this->_buyer_uid and kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['warning_you_no_rights_confirm'],"alert_error");
		...
		if ($this->_agree_status == 2 && $this->_seller_status == 3 && $this->_buyer_status == 3) {
				...
				if ($cash_info['task_union']) {
					...
					$u->task_close ( array ('r_task_id' => $u->_r_task_id, 'indetify' => 1, 'bid_uid' =>$this->_task_info ['uid']) );
				}
				...
				if($res){
					kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['source_file_confirm_deliver_success'],"alert_right");
				
				}else{
					kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['file_confirm_fail_deliver_fail'],"alert_error");
			
				}
			  
		} else {
			kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['current_status_can_not_confirm'],"alert_error");
