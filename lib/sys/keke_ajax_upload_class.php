	...
	public function upload_file() {
		...
		if (is_array ( $savename )) {
			...
			if ($this->_file_type == 'link') {
				$msg = array ('url' => $file_pic . ',' . $real_file,'filename' => $filename, 'localname' => $real_file, 'id' => '1', 'up_file' => $file_pic );
			} else if ($this->_file_type == 'att' || $this->_file_type == 'sys') {
				$msg = array ('url' => $file_pic,'filename' => $filename, 'localname' => $real_file, 'id' => '1', 'up_file' => $file_pic );
			} else {
				$msg = array ('url' => '!' . $file_pic,'filename' => $filename, 'localname' => $real_file, 'id' => '1', 'up_file' => $file_pic );
			}
