...
class keke_shop_class {
	...
	public static function access_check($sid, $s_uid, $model_id) {
		...
		if ($uid==$s_uid||$uid==$order_info['order_uid']) {
			...
			kekezu::keke_show_msg ( "index.php?do=user&view=employer&op=shop&model_id=" . $model_id . "&order_id=" . $order_id, '您没有权限进入此页面', 'error' );
		}
	}
	public static function create_service_order($service_info) {
		...
		if ($order_id) {
			keke_order_class::create_order_detail ( $order_id, $order_name, 'service', intval ( $service_info [service_id] ), $service_cash );
				...
				kekezu::show_msg ( $_lang ['operate_notice'], "index.php?do=shop_order&sid=".$service_info ['service_id']."&steps=step1&order_id=$order_id", '1', $_lang ['order_produce_success'], 'alert_right' );
		} else {
	...
	public static function set_report($obj_id, $to_uid, $to_username, $report_type, $file_name, $desc,$reason) {
		global $uid;
		global $_lang;
		...
		$user_type = '2'; 
		$res = keke_report_class::add_report ( 'product', $obj_id, $to_uid, $to_username, $desc, $report_type, $service_info ['service_status'], $obj_id, $user_type, $file_name ,$reason );
	}
