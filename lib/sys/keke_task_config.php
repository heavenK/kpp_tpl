...
class keke_task_config {
	...
	public static function task_audit_pass($task_ids) {
		...
		if ($task_ids && is_array ( $task_ids )) {
			$ids = implode ( ',', $task_ids );
			$task_arr = db_factory::query ( sprintf ( "select task_id,model_id,task_title,task_cash,task_status,uid,username,start_time,sub_time,end_time,payitem_time from %switkey_task where task_id in(%s) and task_status in (0,1)", TABLEPRE, $ids ) );
			foreach ( $task_arr as $v ) {
				...
				kekezu::save_weibo_data($v['task_id'],'task',$_model_info['model_code'],$v['task_title'],$v['uid'],$v['username'],0,0,0,'pub',$task_cash,$v['uid'],$v['username']);
				$info = db_factory::get_one('select union_user,union_assoc from '.TABLEPRE.'witkey_space where uid='.$v['uid']);
				keke_union_class::union_task_submit($info,$v['task_id']);
			...
			if ($task_info) {
				...
				kekezu::save_weibo_data($task_info['task_id'],'task',$_model_info['model_code'],$task_info['task_title'],$task_info['uid'],$task_info['username'],0,0,0,'pub',$task_cash,$task_info['uid'],$task_info['username']);
				$info = db_factory::get_one('select union_user,union_assoc from '.TABLEPRE.'witkey_space where uid='.$task_info['uid']);
				keke_union_class::union_task_submit($info,$task_info['task_id']);
	...
	public static function task_audit_nopass($task_ids,$trust_response=false) {
		...
		if ($task_ids && is_array ( $task_ids )) {
			...
			foreach ( $task_arr as $v ) {
				...
				db_factory::execute(sprintf("delete from %switkey_payitem_record where obj_id=%d and use_type='spend'",TABLEPRE,$v ['task_id']));
				...
				if($task_config['defeated'] == 1){
					keke_finance_class::cash_in ( $v ['uid'], $return_cash, 0, 'task_fail', 'admin', 'task', $v ['task_id'] );
				}else{
					keke_finance_class::cash_in ( $v ['uid'], 0, $return_cash, 'task_fail', 'admin', 'task', $v ['task_id'] );
				}
			...
			if ($task_info) {
				db_factory::execute(sprintf("delete from %switkey_payitem_record where obj_id=%d and use_type='spend'",TABLEPRE,$ids));
				if($task_info['is_trust']&&$trust_response==false){
					...
					switch($task_info['is_trust']){
						...
						case "0":
							...
							keke_finance_class::init_mem('task_fail', $data);
							if($task_config['defeated'] == 1){
								keke_finance_class::cash_in ( $task_info ['uid'], $return_cash, 0, 'task_fail', 'admin', 'task', $task_info ['task_id'] );
							}else{
								keke_finance_class::cash_in ( $task_info ['uid'], 0, $return_cash, 'task_fail', 'admin', 'task', $task_info ['task_id'] );
							}
							break;
					}
	...
	public static function del_sign_task($task_id,$model){
		...
		public static function can_operate($status,$is_top) {
			...
			switch ($status) {
				...
				case "2" : 
					if($is_top==1){
						$operate ['unrecommend'] = '取消推荐';
					}else{
						$operate ['recommend'] = '推荐';
					}
				case "3" : 
				case "4" : 
				case "5" : 
