...
class keke_user_mark_class {
	...
	public static function exec_mark($mark_id, $content, $mark_status = '1', $aid = null, $aid_star = null,$url,$task_id) {
		global $_lang,$_K;
		$res = self::exec_mark_process($mark_id, $content, $mark_status, $aid, $aid_star);
		if($url){
			$url = strtr($url,"-","=");
			$url = strtr($url,'*','&');
			$url = $_K ['siteurl'].'/index.php?'.$url;
		}elseif($task_id){
			$url = "index.php?do=task&task_id=$task_id&view=mark";
		}
		if($res){
			kekezu::show_msg($_lang['operate_tips'],$url,1,$_lang['congratulation_you_mark_success'],"alert_right");
		}else{
			kekezu::show_msg($_lang['operate_tips'],$url,1, $_lang['sorry_mark_fail'],"alert_error");
		}	
	}	
