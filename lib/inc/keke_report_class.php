...
abstract class keke_report_class {
	...
	public static function check_if_report($type, $obj, $obj_id, $uid, $to_uid) {
		...
		if (! $if_report) {
			...
			kekezu::keke_show_msg ( '', '您已成功提交了' . $trans_name . "信息，我们会尽快处理，请勿重复操作。", "error", 'json' );
		}
	}
			...
			if ($report_satus == '1') { 
				switch ($obj) {
					...
					case "work" :
						...
						break;
					case "order" :
						$order_info = db_factory::get_one ( sprintf ( " select order_name origin_title, model_id from %switkey_order where order_id='%d'", TABLEPRE, $origin_id ) );
						$res = db_factory::execute ( sprintf ( " update %switkey_order set order_status='arbitral' where order_id='%d' ", TABLEPRE, $origin_id ) );
						$re_type = $_lang ['order'];
						if($uid==$order_info['seller_uid']){
							$re_obj = "<a href=\"" . $_K ['siteurl'] . "/index.php?do=user&view=employer&op=shop&model_id=".$order_info['model_id']."\">" . $order_info ['origin_title'] . "</a>";
						}else{
							$re_obj = "<a href=\"" . $_K ['siteurl'] . "/index.php?do=user&view=witkey&op=shop&model_id=".$order_info['model_id']."\">" . $order_info ['origin_title'] . "</a>";
						}
						break;
					case "product" :
						break;
	...
	public static function add_report($obj, $obj_id, $to_uid, $to_username, $desc, $report_type, $front_status = null, $origin_id = null, $user_type = null, $file_name = NULL,$reason=NULL, $is_hide = 1) {
			...
			switch ($obj) {
				case 'task':
					...
					$result = array (
							...
							'类型' => '任务',
							'标题'=>'<a href="index.php?do=task&task_id='.$task_info['task_id'].'">'.$task_info['task_title'].'</a>',
				...
				case 'product':
					...
					$result = array (
							...
							'模型名称' => $model_info['model_name'],
							'类型' => '商品（服务）',
							'标题'=>'<a href="index.php?do=task&task_id='.$service_info['task_id'].'">'.$service_info['task_title'].'</a>',
							$_lang['website_name'] => $kekezu->_sys_config['website_name']
					);
				break;
			}
			$result and keke_msg_class::notify_user ( $to_uid, $to_username, 'report_notice', '举报通知', $result );
			kekezu::keke_show_msg ( '', '感谢您的正义'.$transname.'，管理员会尽快受理，请耐心等待处理结果。', "", 'json' );
		} else {
	...
	public static function get_report_employee_reason(){
		return  array('sxcx'=>'涉嫌抄袭','waswcgz'=>'未按时完成工作','jjxgzp'=>'拒绝修改作品','yqzjsj'=>'要求追加赏金','wnlwcyq'=>'无能力完成要求','qt'=>'其它');
	}
	public static function get_report_witkey_reason(){
		return  array('sxzb'=>'涉嫌作弊','shyqbhl'=>'审核要求不合理','jfsj'=>'拒付赏金','qt'=>'其它');
	}
