		...
		switch ($this->_sys_config ['user_intergration']) {
			case "1" :
				...
				if ($user_info ['password'] !== $password) {
					...
					$this->show_msg ( $_lang['you_input_username_or_password_not_exist'], 3,$url,1 ); 
		...
		switch ($this->_login_type) {
			...
			case "1" :
				if($_K['inajax']){
					...
					$status==1 and $type = 'success' or $type='warning';
				}
				kekezu::show_msg ( $_lang ['friendly_notice'],$url, 2, $content,$type);
				break;
		}
	}
}