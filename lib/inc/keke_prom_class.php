...
class keke_prom_class {
	...
	function create_prom_cookie($query_string) {
		...
		if ($uid) { 
			...
			setcookie ( "user_prom_event", serialize ( $url_data ), time () + 24 * 3600, COOKIE_PATH, COOKIE_DOMAIN, NULL, TRUE );
		}
	...
	public function is_meet_requirement($prom_code, $obj_id) {
		...
		if ($obj_info) {
			...
			if ($prom_config ['indus_string'] && !in_array($obj_info ['indus_id'],$config_indus_array)) {
				$result = FALSE;
			}
